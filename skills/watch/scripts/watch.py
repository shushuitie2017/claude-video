...
Transcript strategy (three tiers):
  1. native captions via yt-dlp (free, fastest)
  2. local faster-whisper if installed (free, word-accurate)
  3. Groq / OpenAI Whisper API (needs a key)
"""
...
from whisper_api import extract_audio, load_api_key, transcribe_video  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        "--no-whisper",
        action="store_true",
        help="Disable the Whisper fallback (local AND API). Report frames-only if no captions available.",
    )
    ap.add_argument(
        "--no-local-whisper",
        action="store_true",
        help="Skip the local faster-whisper tier; go straight to the Whisper API fallback.",
    )
    ap.add_argument(
        "--whisper",
        choices=["groq", "openai"],
        default=None,
        help="Force a specific Whisper API backend (skips the local tier). "
             "Default: local faster-whisper first, then prefer Groq over OpenAI.",
    )
    ap.add_argument(
        "--browser",
        default="chrome",
        help="Browser to read cookies from when yt-dlp gets blocked (default chrome; 'none' disables).",
    )
    ap.add_argument(
        "--no-dedup",
        action="store_true",
    ...
    if url_source:
        ...
        dl = fetch_captions(args.source, work / "download", browser=browser)
    ...
    # Tier 2: local faster-whisper (free, no key needed). Skipped when the user
    # forces an API backend with --whisper, or passes --no-local-whisper.
    if (
        not transcript_segments and not args.no_whisper and not args.no_local_whisper
        and args.whisper is None and video_path and meta.get("has_audio")
        and local_whisper.is_available()
    ):
        try:
            print("[watch] no captions — transcribing with local faster-whisper…", file=sys.stderr)
            audio_path = extract_audio(video_path, work / "audio.mp3")
            all_segments = local_whisper.transcribe_segments(str(audio_path))
            if all_segments:
                transcript_segments = filter_range(all_segments, start_sec, end_sec) if focused else all_segments
                transcript_text = format_transcript(transcript_segments)
    ...
    # Tier 3: Whisper API (Groq preferred, OpenAI fallback).
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        ...
        if backend and api_key:
            ...
            print(
                ...
                "(or `pip install faster-whisper` for the free local tier)",
                file=sys.stderr,
            )
    ...
    if transcript_text:
        ...
        print(
            ...
            "(no local faster-whisper, no API key set, or `--no-whisper` was used). "
