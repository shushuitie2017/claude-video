...
srt 解码器剥离（实测三档字号 md5 全同）。只有真正的 ASS 文件
...
字体按平台自动选（Windows 微软雅黑 / macOS 苹方 / Linux Noto），
韩语 / 阿拉伯语 / 日语假名按主导文字脚本切对应系统字体，见 fonts.py。
"""
...
from fonts import script_font_table, subtitle_font_name  # noqa: E402
...
def build_ass(items, cn_size, en_size, font=None, marginv=16):
    if font is None:
...
def main():
    ...
    ap.add_argument("--font", default=None, help="字体名（默认按平台+文字脚本自动选，见 fonts.py）")
